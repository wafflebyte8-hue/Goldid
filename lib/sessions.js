...
function save(id, messages, extra = {}) {
  ...
  try {
    createdAt =
      JSON.parse(fs.readFileSync(file, "utf8")).createdAt || createdAt;
  } catch {
    /* new session */
  }
  const data = {
    version: 1,
    id: clean,
    title: extra.title || titleFrom(messages),
...
module.exports = { SESSION_DIR, newId, save, load, list, search, remove };
