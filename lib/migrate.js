...
function copyItem(item, overwrite) {
  ...
  if (exists(item.to) && !overwrite) {
    if (item.kind === 'skill') skills.writeVersionManifest(item.to, item.source, false);
    return 'skipped';
  }
  ...
  if (item.kind === 'skill') skills.writeVersionManifest(item.to, item.source, true);
  return 'copied';
}
