...
async function installFromRegistry(id, opts = {}) {
  ...
  return {
    ...
    slug: installed?.slug || path.basename(dir).toLowerCase(),
    dir,
    version: installed?.version || item.version || '',
  };
}

async function registrySkillDetail(id, opts = {}) {
  const clean = safeSkillId(id);
  const registry = await fetchRegistry(opts.registryUrl || defaultRegistryUrl());
  const item = registry.skills.find((skill) => skill.id.toLowerCase() === clean);
  if (!item) throw new Error(`skill not found in registry: ${clean}`);
  ...
  const versionUrl = absoluteUrl(registry.baseUrl, item.versionUrl || `skills/${clean}/Version.js`);
  const [skillMd, versionSourceText] = await Promise.all([fetchTextUrl(skillUrl), fetchTextUrl(versionUrl)]);
  const manifest = parseVersionSource(versionSourceText);
  const parsed = parseFrontmatter(skillMd);
  return {
    ...item,
    name: String(manifest.Name || item.name || clean).trim(),
    description: String(manifest.Description || item.description || parsed.metadata.description || '').trim(),
    author: String(manifest.Author || item.author || '').trim(),
    usage: String(manifest.Usage || parsed.metadata.usage || '').trim(),
    modelTested: modelList(manifest.Model_tested),
    versionSource: versionSourceText,
    skillPreview: parsed.body.slice(0, 4000),
  };
}

function uninstallInstalled(name, cwd = process.cwd()) {
  const skill = find(name, cwd);
  if (!skill) throw new Error('Skill not found');
  const installRoot = path.resolve(skillInstallRoot());
  const target = path.resolve(skill.dir);
  const relative = path.relative(installRoot, target);
  if (!relative || relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error('Only installed marketplace skills can be uninstalled here');
  }
  fs.rmSync(target, { recursive: true, force: true });
  return { name: skill.name, slug: skill.slug, dir: target };
}
...
function catalog(cwd = process.cwd()) {
  ...
  return [
    '# Available skills',
    '',
    'Skills are reusable instruction documents. If the user asks for something',
    'and one available skill exactly matches that task, you MUST call skill_view',
    'for that skill before acting. Use the loaded skill workflow as the primary',
    'workflow for the task. Do not claim to have used a skill unless you loaded it.',
    'Project skills override user and imported skills.',
    '',
...
function listResult(cwd = process.cwd()) {
  return discover(cwd).map(({ name, slug, description, version, author, usage, modelTested, platforms, source, file, dir }) => ({
    name,
    slug,
    description,
    version,
