...
async function installFromRegistry(id, opts = {}) {
  ...
  const versionUrl = absoluteUrl(registry.baseUrl, item.versionUrl || `skills/${clean}/Version.md`);
  const [skillMd, versionMd] = await Promise.all([fetchTextUrl(skillUrl), fetchTextUrl(versionUrl)]);
  const dir = path.join(skillInstallRoot(), clean);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'SKILL.md'), skillMd, 'utf8');
  fs.writeFileSync(path.join(dir, VERSION_MD_FILE), versionMd, 'utf8');
  writeVersionManifest(dir, 'goldid', true);
