...
function modelContextFromJson(json, model) {
  const direct = json.context_length || json.contextLength || json.inputTokenLimit || json.input_token_limit;
  ...
  const found = data.find((m) => {
    ...
    return id === model || m.name === model || m.name === `models/${model}`;
  });
  if (found) return modelContextFromJson(found, model);
  const info = json.model_info || json.details || {};
  for (const value of Object.values(info)) {
    if (Number.isFinite(Number(value)) && /context/i.test(String(Object.keys(info).find((k) => info[k] === value) || ''))) {
      return Number(value);
    }
  }
  return null;
...
async function fetchModelContextLength(key, conf, model) {
  const def = PROVIDERS[key];
  ...
  if (key === 'ollama') {
    try {
      const json = await fetchJson(baseUrl(def, conf) + '/api/show', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: model }),
      });
      return modelContextFromJson(json, model);
    } catch {
      return null;
    }
  }
  ...
  if (def.parse === 'gemini') {
    try {
      const url = `${baseUrl(def, conf)}/v1beta/models/${model}?key=${encodeURIComponent(conf.apiKey || '')}`;
      return modelContextFromJson(await fetchJson(url, { headers: { 'Content-Type': 'application/json' } }), model);
    } catch {
      return null;
    }
  }
  ...
  try {
    return modelContextFromJson(await fetchJson(modelsUrl(def, conf), { headers }), model);
  } catch {
    return null;
  }
}
...
module.exports = {
  PROVIDERS, fetchModels, fetchModelContextLength, chat, chatStream, generateImage, baseUrl,
