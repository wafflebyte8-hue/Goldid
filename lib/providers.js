...
async function chat(key, conf, model, messages, opts = {}) {
  ...
  if (def.chat === 'gemini') {
    ...
    const json = await fetchJson(url, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), timeout: 120000,
    });
  ...
  const json = await fetchJson(baseUrl(def, conf) + def.chatPath, {
    method: 'POST', headers: authHeaders(def, conf), body: JSON.stringify({ model, messages: oaMsgs }), timeout: 120000,
  });
...
async function generateImage(key, conf, model, prompt, opts = {}) {
  ...
  const body = { model: imageModel, prompt, n: 1, response_format: 'b64_json' };
 ...
 */
async function chatStream(key, conf, model, messages, { system, onDelta, tools } = {}) {
  const def = PROVIDERS[key];
  if (!def) throw new Error(`unknown provider: ${key}`);
  const msgs = asMessages(messages);
  ...
  if (def.chat === 'gemini') {
    ...
    const res = await fetchStream(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  if (def.chat === 'anthropic') {
    ...
    if (system) body.system = system;
    const res = await fetchStream(baseUrl(def, conf) + def.chatPath, { method: 'POST', headers: authHeaders(def, conf), body: JSON.stringify(body) });
    for await (const line of sseLines(res)) {
      if (!line.startsWith('data:')) continue;
      const data = line.slice(5).trim();
      if (!data) continue;
  ...
  const res = await fetchStream(baseUrl(def, conf) + def.chatPath, {
    method: 'POST', headers: authHeaders(def, conf), body: JSON.stringify(body),
  });
  let reasoning = '';
...
module.exports = { PROVIDERS, fetchModels, chat, chatStream, generateImage, baseUrl };
