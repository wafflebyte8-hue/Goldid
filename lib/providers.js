...
async function chatStream(key, conf, model, messages, { system, onDelta, tools, signal } = {}) {
  ...
  if (def.chat === 'gemini') {
    ...
    const res = await fetchStream(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal });
  ...
  if (def.chat === 'anthropic') {
    ...
    if (system) body.system = system;
    const res = await fetchStream(baseUrl(def, conf) + def.chatPath, { method: 'POST', headers: authHeaders(def, conf), body: JSON.stringify(body), signal });
    for await (const line of sseLines(res)) {
      if (!line.startsWith('data:')) continue;
      const data = line.slice(5).trim();
      if (!data) continue;
  ...
  const res = await fetchStream(baseUrl(def, conf) + def.chatPath, {
    method: 'POST', headers: authHeaders(def, conf), body: JSON.stringify(body), signal,
  });
  let reasoning = '';
