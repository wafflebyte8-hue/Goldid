...
function linuxTpmAvailable() {
  return process.platform === 'linux' && linuxHasTpmDevice() && hasCmd('tpm2_unseal');
}
...
function migrateToTpm() {
  ...
  if (isTpmAvailable()) {
    ...
    if (!check.equals(key)) {
      fs.rmSync(WRAPPED_KEY, { force: true });
      return { ok: false, changed: false, message: 'TPM round-trip verification failed; nothing changed.' };
    }
    cleanupAfter('tpm');
    cachedKey = key;
  ...
  // No TPM: machine-bound double-encrypted hidden store.
  writeMachineStore(key);
  const check = machineUnwrap(fs.readFileSync(MACHINE_KEY, 'utf8'));
  if (!check.equals(key)) {
    fs.rmSync(MACHINE_KEY, { force: true });
    return { ok: false, changed: false, message: 'Machine-store verification failed; nothing changed.' };
  }
  cleanupAfter('machine');
  cachedKey = key;
  return { ok: true, changed: true, message: 'No TPM — master key double-encrypted and hidden, bound to this machine. key.bin removed.' };
}
