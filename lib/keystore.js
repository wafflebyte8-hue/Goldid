...
function linuxSeal(keyBuf) {
  ...
  try {
    ...
    // Feed the key over stdin ('-i -') so the plaintext never lands in a file.
    execFileSync('tpm2_create', ['-C', primary, '-g', 'sha256', '-i', '-',
      '-u', path.join(tmp, 'seal.pub'), '-r', path.join(tmp, 'seal.priv')],
      { input: keyBuf, stdio: ['pipe', 'ignore', 'ignore'] });
 ...
 *
 * Critical invariant: a key store that EXISTS is authoritative. If it is present
 * but cannot be recovered (e.g. a transient TPM failure), we throw a hard error
 * and change nothing — we never mint a replacement key, because doing so would
 * orphan the already-encrypted config.json. "No key exists" and "a key exists but
 * could not be recovered" are different situations and must not be conflated.
 */
function getMasterKey() {
  ...
  if (fs.existsSync(WRAPPED_KEY)) {
    let key;
    try {
      key = unsealWithTpm(fs.readFileSync(WRAPPED_KEY, 'utf8'));
    } catch (e) {
      throw new Error(
        `Master key is sealed in the TPM but could not be unsealed (${e.message}). ` +
        'Refusing to create a new key so your existing encrypted secrets stay intact. ' +
        'This is usually transient — ensure the TPM is available and try again.'
      );
    }
    if (key.length !== KEY_BYTES) throw new Error('TPM-unsealed master key has an unexpected length; refusing to proceed.');
    cachedKey = key;
    return key;
  }
  ...
  if (fs.existsSync(MACHINE_KEY)) {
    let key;
    try {
      key = machineUnwrap(fs.readFileSync(MACHINE_KEY, 'utf8'));
    } catch (e) {
      throw new Error(
        `Machine-bound master key exists but could not be decrypted (${e.message}). ` +
        'Refusing to create a new key. If you moved machines, restore the original file or re-enter your API keys.'
      );
    }
    if (key.length !== KEY_BYTES) throw new Error('Machine-bound master key has an unexpected length; refusing to proceed.');
    cachedKey = key;
    return key;
  }

  if (fs.existsSync(PLAIN_KEY)) {
    const key = fs.readFileSync(PLAIN_KEY);
    if (key.length === KEY_BYTES) { cachedKey = key; return key; }
    throw new Error('Legacy key.bin exists but is malformed; refusing to overwrite it.');
  }

  // Truly no key exists anywhere — safe to create one and protect it.
