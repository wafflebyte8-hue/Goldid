...
function toolBlockDetailed() {
  return [
    ...
    ...toolSummaryLines(),
    "",
    'When the user asks about the "project graph", repository graph, file graph, dependencies, or connected files, use the project_graph tool directly. Do not say you lack a project graph.',
    "",
    "### How to call a tool",
    "",
  ...
  return [
    ...
    "Tools: " +
      toolSummaryLines()
        ...
        .join(", ") +
      ".",
    'Use project_graph for project graph, repository graph, dependency graph, and connected-file questions.',
...
function cloudLines({
  ...
  L.push(`Chat naming mode is "${namingMode}".`);
  L.push("");
  L.push("# Core mandate");
  L.push("");
  L.push(
  ...
  L.push("");
  L.push(
    "GolDid supports generated chat/session names through /name never, /name auto,",
  );
  L.push(
    "and /name always. never disables generated titles; auto generates a title only",
  );
  L.push(
    "when the completed response is faster than 10 estimated tokens per second;",
  );
  L.push(
    "always generates after each completed chat response. The current mode is",
  );
  L.push(`"${namingMode}".`);
  L.push(
    "Do not print, announce, or invent a chat title inside normal answers unless the",
  );
  L.push(
    "user explicitly asks for one. The app/CLI handles naming after the response.",
  );
  L.push(
    "If the user asks to change this behavior, use or explain the /name mode command",
  );
  L.push("instead of editing session files by hand.");
  return L;
}
...
function localLines({
  ...
  L.push("");
  L.push(
    "Chat naming: /name never disables generated names, /name auto names only when",
  );
  L.push(
    "the completed response is above 10 estimated tokens per second, and /name always",
  );
  L.push(
    "names after every completed chat response. Do not add title text to normal",
  );
  L.push(
    "answers unless the user explicitly asks; GolDid handles title generation after",
  );
  L.push("the reply.");
  L.push("");
...
function buildSystemPrompt({
  ...
  lines.push(
    ...
    "## Chat termination",
    "",
    "You may proactively end the current chat session when continuing would be inappropriate or impossible.",
    "Use this only for clear triggers: the user asks to end/stop the chat, sustained harassment or abuse, attempts to force protocol violations, or requests that require breaking safety/security rules.",
    "When ending the chat, write any brief final message the user should see, then include exactly one control token at the end:",
    '<end_chat reason="user_request|harassment|protocol_violation|safety|other"/>',
    "Do not use the token for ordinary disagreement, uncertainty, or normal task completion."
  );
  return lines.join("\n");
}
