...
const projectGraph = require('./project-graph');
...
const TOOLS = {
  ...
  project_graph: {
    desc: 'Inspect the current repository project graph: files as nodes and import/dependency links as edges.',
    args: {
      action: 'summary, search, or neighbors (default "summary")',
      query: 'file path/name substring for search or neighbors',
      max_results: 'maximum results to return (default 20)',
    },
    required: [],
    danger: false,
    run: async ({ action = 'summary', query = '', max_results = '20' }) => {
      const graph = projectGraph.build(process.cwd());
      const limit = asInt(max_results, 20, 100);
      ...
      const top = [...degree.entries()]
        ...
        .slice(0, limit);
      const mode = String(action || 'summary').toLowerCase();
      ...
      if (mode === 'search') {
        ...
        const matches = graph.nodes
          ...
          .slice(0, limit)
          .map((n) => `${n.id} (${degree.get(n.id) || 0} links)`);
        return clip(matches.join('\n') || `(no graph nodes match "${query}")`);
      }

      if (mode === 'neighbors') {
        if (!q) throw new Error('query is required for neighbors');
        const node = graph.nodes.find((n) => n.id.toLowerCase() === q) ||
          graph.nodes.find((n) => n.id.toLowerCase().includes(q));
        if (!node) return `(no graph node matches "${query}")`;
        ...
        return clip([
          `node: ${node.id}`,
          `links: ${degree.get(node.id) || 0}`,
          '',
          'imports:',
          outgoing.length ? outgoing.slice(0, limit).join('\n') : '(none)',
          '',
          'imported by:',
          incoming.length ? incoming.slice(0, limit).join('\n') : '(none)',
        ].join('\n'));
      }
      ...
      return clip([
        ...
        top.length ? top.map(([id, count]) => `${id} (${count} links)`).join('\n') : '(none)',
      ].join('\n'));
    },
  },
  web_search: {
  ...
  generate_image: {
    desc: 'Generate an image from a text prompt and save it to a file (needs an image-capable provider/model).',
    args: {
      prompt: 'a detailed description of the image to generate',
      path: 'output file path (default goldid-image-<timestamp>.png)',
      model: 'image model id to use (optional; a provider default is used otherwise)',
    ...
    run: async (args, ctx = {}) => {
      ...
      if (typeof ctx.generateImage !== 'function') {
        throw new Error('image generation is unavailable in this context');
      }
      const img = await ctx.generateImage(prompt, { model: args.model, size: args.size });
      let buf;
      if (img.b64) {
      ...
      const out = args.path || `goldid-image-${Date.now()}.${ext}`;
  ...
  shell: {
    ...
    run: ({ command }, ctx = {}) =>
      new Promise((resolve, reject) => {
        ...
        // ctx.wrapShell (supplied by goldid.js) applies the active sandbox:
        // pins the cwd in jail mode or wraps the command in a container in
        // docker mode. Absent (or sandbox off) it runs the command as-is.
        let toRun = command;
        let opts = {};
        if (typeof ctx.wrapShell === 'function') {
          try {
            ({ command: toRun, options: opts } = ctx.wrapShell(command));
          } catch (e) {
            return reject(e);
          }
        }
        exec(
          toRun,
          { timeout: 60000, windowsHide: true, maxBuffer: 1024 * 1024, ...opts },
          (err, stdout, stderr) => {
...
/**
 * Extract the <end_chat reason="..."/> control token from a final reply (see
 * the "Chat termination" section of the system prompt). Returns
 * { text, ended } where `text` is the reply without the token and `ended` is
 * null or { reason }. Shared by the CLI and the desktop app.
 */
...
module.exports = { TOOLS, toolSchemas, toolSummaryLines, parseToolCall, parseEndChat, clip };
